...
collect() {
  ...
  "${compose[@]}" ps --all > "$result_dir/containers.txt" 2>&1 || true
  for service in "${services[@]}" postgres redis frontend zipkin loki mailpit; do
    "${compose[@]}" logs --no-color "$service" > "$result_dir/$service.log" 2>&1 || true
  done
  if [[ "$gate_exit" != 0 ]]; then echo "${mode^^}_STACK_GATE_FAIL" > "$result_dir/failure-classification.txt"; fi
...
sample_memory() {
  ...
  [[ "${#backend_ids[@]}" == 8 ]]
  docker stats --no-stream --format '{{json .}}' "${backend_ids[@]}" > "$result_dir/backend-$stage-stats.jsonl"
  bytes="$(jq -s '
...
verify_hmac() {
  ...
  unsigned="$(curl -sS -o "$result_dir/$service-unsigned.json" -w '%{http_code}' "${args[@]}" "$url")"
  [[ "$unsigned" == 401 ]]
  ...
  first="$(curl -sS -o "$result_dir/$service-signed.json" -w '%{http_code}' "${args[@]}" "${headers[@]}" "$url")"
  replay="$(curl -sS -o "$result_dir/$service-replay.json" -w '%{http_code}' "${args[@]}" "${headers[@]}" "$url")"
  [[ "$first" == "$accepted" && "$replay" == 401 ]]
  [[ "$("${compose[@]}" exec -T redis redis-cli --no-auth-warning -a "$REDIS_PASSWORD" exists "internal-auth:nonce:$nonce")" == 1 ]]
...
"${compose[@]}" pull postgres redis zipkin loki mailpit
started_ms="$(date +%s%3N)"
"${compose[@]}" up -d --no-build --wait --wait-timeout 600 postgres redis "${services[@]}" frontend zipkin loki mailpit
...
for index in "${!services[@]}"; do
  ...
  curl -fsS "http://127.0.0.1:${management_ports[$index]}/actuator/prometheus" > "$result_dir/$service-prometheus.txt"
...
for profile in auth-service api-gateway guest-service frontdesk-service billing-service fb-service notification-service; do
  curl -fsS -u "configuser:$CONFIG_SERVER_PASSWORD" "http://127.0.0.1:18888/$profile/default" \
...
load_sampler_pid=$!
(cd frontend && npx playwright test --config playwright-native.config.ts)
kill "$load_sampler_pid" 2>/dev/null || true
wait "$load_sampler_pid" 2>/dev/null || true
load_sampler_pid=''
...
curl -fsS http://127.0.0.1:19411/api/v2/services > "$result_dir/zipkin-services.json"
